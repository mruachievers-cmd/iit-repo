...
class DialogueSystem {
        ...
        switch (npc.type) {
            ...
            case 'receptionist':
                if (!name) {
                    ...
                    this.addTextInput("What is your name, candidate?", (nameInput) => {
                        this.memory.setPlayerName(nameInput);
                        this.addTextInput("Please enter your Gmail for verification.", (emailInput) => {
                            this.memory.setPlayerEmail(emailInput);
                            const mockOTP = "4921";
                            this.addLine(`[PHONE]: New message - Your COMPANY OTP is ${mockOTP}`);
                            this.addTextInput("Enter the OTP sent to your email.", (otpInput) => {
                                if (otpInput.trim() === mockOTP) {
                                    this.memory.setOtpVerified();
                                    this.addLine(`Authentication complete, ${nameInput}. You are scheduled for Round 1: Group Discussion.`);
                                    this.addLine("Please proceed through the double doors and find Room A on the left.");
                                } else {
                                    this.addLine("Incorrect OTP. We will assume technical difficulties. Proceed to Round 1 anyway.");
                                }
                                this.processNextLine();
                            });
                            this.processNextLine();
            ...
            case 'guard':
            case 'candidate':
            case 'watercooler_emp':
            case 'senior':
                this.addLine("Talk to me...");
                this.startChatLoop(npc);
                break;
            case 'sofa':
                if (this.memory.flags.roundsPassed === 1 && this.memory.flags.bossesDefeated === 0) {
                    this.addLine("The lobby sofa looks surprisingly comfortable.");
                    this.addChoice("[Take a Nap?]", ["Sleep"], () => {
                        this.addLine("You close your eyes... the world begins to warp...");
                        this.addChoice("[???]", ["Enter Dream"], () => {
                            window.game.startDreamSequence();
                            this.processNextLine();
                        });
                        this.processNextLine();
                    });
                } else {
                    this.addLine("It's a sofa. Now is not the time to rest.");
                }
                break;
            case 'candidate':
                if (npc.id === 'nervousC') {
            ...
            case 'tree':
                ...
                this.addChoice("[What do you do?]", ["🔓 HACK THE SERVER"], () => {
                    ...
                    this.addLine("Wait... you realise this isn't a dream anymore. You are really going to fight a giant!");
                    this.addChoice("[Prepare for Impact]", ["⚔️ FIGHT FOR REAL"], () => {
                        this.memory.flags.roundsPassed = 2;
                        this.memory.save();
                        npc.active = false;
                        window.game.startTreeFightSequence();
                        this.processNextLine();
                    });
                    this.processNextLine();
                });
            ...
            this.addChoice("Your Response:", [
                ...
                this.addChoice("Conclude GD?", ["View Results"], () => {
                    ...
                    if (score > 4) {
                        ...
                        this.addLine("[GD Moderator]: Excellent work. Please go take a short nap on the Sofa in the lobby before Round 2.");
                        this.addChoice("[Leave Room]", ["Understood"], () => {
                            this.memory.flags.roundsPassed = 1;
                            this.memory.save();
                            const mod = window.game.npcs.find(n => n.id === 'moderator');
    ...
    startChatLoop(npc) {
        this.addTextInput("Type your message (or 'exit' to end):", async (input) => {
            if (input.toLowerCase() === 'exit' || input.toLowerCase() === 'quit' || input.toLowerCase() === 'bye') {
                this.addLine("Goodbye.");
                this.processNextLine();
                return;
            }

            this.addLine("Thinking...");
            this.processNextLine();

            try {
                const response = await this.ai.chat(npc.id, npc.type, input);
                // Replace "Thinking..." with the actual response
                this.lines.pop(); // Remove "Thinking..."
                this.addLine(`[AI]: ${response}`);
                this.startChatLoop(npc); // Continue loop
                this.processNextLine();
            } catch (err) {
                this.addLine("My brain is foggy... check the console.");
                this.processNextLine();
            }
        });
    }
