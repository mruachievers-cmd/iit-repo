...
class NPC {
    ...
    draw(ctx, camX, camY) {
        ...
        if (this.type === 'sofa') {
            ...
            ctx.fillRect(px - 8, py + 8, 48, 12);
            // Sofa seating
            ctx.fillStyle = '#506070';
            ctx.fillRect(px - 8, py + 20, 48, 14);
            // Armrests
            ctx.fillStyle = '#304050';
            ctx.fillRect(px - 10, py + 12, 6, 22);
            ctx.fillRect(px + 36, py + 12, 6, 22);
            ctx.restore();
            this.drawNameLabel(ctx, px + 8, py + 8);
            return;
        }
        if (this.type === 'tree') {
            ctx.save();
...
function createNPCs() {
    return [
        ...
        new NPC({ id: 'lobby_sofa', name: 'Lobby Sofa', type: 'sofa', x: 28, y: 30, map: 'company' }),
